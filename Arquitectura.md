...
| **RQ-02** | _Protector del software_ | _Proteger las credenciales y datos de clientes frente a algunos códigos maliciosos para engañar al sistema_                    | _Mecanismos de cifrado, base de datos y código_                 | _Pruebas de seguridad_  | _Se debe aplicar cifrado fuerte, validar entradas contra código malicioso._ | _El esfuerzo que se requiere es que deben de trabajarlo hasta que no se encuentren vulnerabilidades_    |
| **RQ-03** | _Conector del software_                 | _El sistema debe de poder agregar más servidors para soportar un 50% de aumento en la demanda mensual_ | _Configuración de la infraestructura y distribuir trabajo para evitar sobrecargas_ | _Entorno de producción_ | _Se debe de implementar un autoescalado y un balanceo dinámico_        | _La infraestructura debe adaptarse automáticamente sin intervención manual._                 
| **RQ-04** | _Diseñador de software_  | _El sistema debe garantizar una alta disponibilidad del servicio, minimizando el tiempo de inactividad._ | _Infraestructura y monitoreo_ | _Producción_   | _Se debe implementar una arquitectura redundante con monitoreo proactivo._ | _El tiempo de inactividad no debe superar 5 minutos al mes._ |
| **RQ-05** | _Desarrollador_  | _Se solicita optimizar el rendimiento del sistema de gestión de órdenes de compra._ | _Código, consultas a la base de datos y configuración de caché._ | _Tiempo de ejecución_   | _Se debe mejorar el tiempo de respuesta de las consultas y procesamiento de órdenes._ | _v_ |
| **RQ-06** | _Desarrollador_  | _Se solicita optimizar el rendimiento del sistema de gestión de órdenes de compra._ | _Código, consultas a la base de datos y configuración de caché._ | _Tiempo de ejecución_   | _Se debe mejorar el tiempo de respuesta de las consultas y procesamiento de órdenes._ | _v_ |
